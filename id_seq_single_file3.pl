...
my %ids_hash;
#-----------------------[reads ids]--------------
   open ID,"$ARGV[0]" or die "Can't open ID file";
   while(<ID>) {
	   chomp;
       next if (exists $ids_hash{$_});
       $ids_hash{$_} = 1;
               }
...
open IN, "$ARGV[1]" or die "Failed to open IN file\n";
open OUT, ">$ARGV[2]" or die "Failed to open OUT file\n";
